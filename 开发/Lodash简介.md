...
#### **📌 1. `_.intersection`（找交集）**

`_.intersection` 返回所有输入数组中都包含的**相同元素**，原样返回元素。
```javascript
let arr1 = [1, 2, 3];
let arr2 = [2, 3, 4];
let arr3 = [3, 4, 5];
...
// => [3] （交集是 3）
```
- **工作方式**：直接查找各数组中**相等的元素**，返回这些元素的交集。

---

#### **📌 2. `_.intersectionBy`（根据条件找交集）**

`_.intersectionBy` 允许你提供一个 **iteratee** 函数（或属性）来对比元素，**根据自定义的条件**找交集。
...
console.log(_.intersectionBy(arr1, arr2, arr3, 'x'));
// => [{ 'x': 2 }] （根据 'x' 属性，交集是 { x: 2 }）
```

- **工作方式**：**通过自定义属性（如 `'x'`）或函数来比较元素**，从而找出交集。

---

#### **📌 3. `_.intersectionWith`（自定义对比函数）**

`_.intersectionWith` 允许你传入一个自定义的 **比较函数** 来控制如何比较元素，**比较函数返回值为 `true` 表示元素相等**。
...
console.log(_.intersectionWith(arr1, arr2, arr3, (a, b) => a === b));
// => [3] （使用自定义函数进行比较，交集是 3）
```

- **工作方式**：通过自定义的比较函数来判断数组中哪些元素应该被视为“相等”，然后返回交集。
...
|方法|作用|说明|
|---|---|---|
|`_.intersection`|**返回所有数组中的交集元素**|比较元素是否完全相等，返回交集。|
|`_.intersectionBy`|**根据自定义属性或函数返回交集元素**|用指定的属性或函数来比较元素，返回交集。|
|`_.intersectionWith`|**根据自定义比较函数返回交集元素**|使用自定义比较函数来判断两个元素是否相等，返回交集。|

---

#### **✅ 适用场景**

- **`_.intersection`**：当你只需要**直接比较数组中的元素**来查找交集时，使用 `_.intersection`。
- **`_.intersectionBy`**：当需要根据某个**特定属性或条件**来找交集时，可以使用 `_.intersectionBy`。
- **`_.intersectionWith`**：当你需要完全自定义**对比规则**时，使用 `_.intersectionWith`。
...
|方法|作用|示例|
|---|---|---|
|`_.intersection`|**直接比较元素的交集**|`_.intersection([1, 2], [2, 3]) => [2]`|
|`_.intersectionBy`|**通过指定的属性或函数比较元素的交集**|`_.intersectionBy([{ x: 1 }, { x: 2 }], [{ x: 2 }], 'x') => [{ x: 2 }]`|
|`_.intersectionWith`|**通过自定义比较函数找交集**|`_.intersectionWith([1, 2], [2, 3], (a, b) => a === b) => [2]`|

这三个方法根据不同的需求提供了灵活的交集查找能力，确保你在处理数据时能够更高效地进行元素比较。
